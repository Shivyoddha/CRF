class GrindingsController < ApplicationController
  ...
  private
    ...
    def grinding_params
      params.require(:grinding).permit(:sample, :diameter, :mould, :grit, :diamond, :suspension, :lapping, :more, :status, :slotdate, :slottime, :debit,:user_id,equipment_table_attributes: [:username, :app_no, :debit_head, :dummy, :pay],references: [])
    end
end
